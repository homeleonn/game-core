...
class SafeMySQL
{
	...
	private function lazyConnect(){
		$opt = $this->defaults;
		@$this->conn = mysqli_connect('127.0.0.1', $opt['user'], $opt['pass'], $opt['db'], $opt['port'], $opt['socket']);
		if ( !$this->conn )
		{
	...
	private function rawQuery($query)
	{
		...
		$this->stats[] = array(
			'query' => $query,
			'start' => $start,
			'timer' => round((microtime(TRUE) - $start) * 1000, 2), //$timer,
		);
		if (!$res)
		{
	...
	function queryTrace($conn, $query){
		...
		$timer = microtime(TRUE) - $start;
		global $savedQuery;$savedQuery[] = array('query' => $query, 'start' => $start, 'timer' => round($timer / 1000, 3));
		return $res;
	}
